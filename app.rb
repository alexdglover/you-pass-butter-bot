...
set protection: [:json_csrf]
set except: [:json_csrf]
...
post '/images' do
  if COMMAND_IMAGE_MAPPING.key?(params['text'])
    ...
    string_as_json_response "Cannot find that image #{params['text']}. Full " \
    "list of images:\n  #{images}"
  end
end
...
post '/memes' do
  ...
  if COMMAND_MEME_MAPPING.key?(text_params[0])
    ...
    string_as_json_response "Cannot find that meme #{text_params[0]}. Full " \
...
def post_image_to_response_url(response_url, image_url)
  message = { attachments: [{ image_url: image_url, fallback: 'Required ' \
